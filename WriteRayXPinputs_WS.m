...
% Normalizes scatteri-00ng matrix from SolLib, generates the mie matrices for
...
%||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
Sim_num = 602;        % Denotes the simulation folder number
...
CHL = 5;        % Chlorophyl concentration in milligrams/m^3

% Wind speed for waves
WS = [3];       %Windspeed in m/s = 0 - no Windy waves [2.5 .. 15] Windy waves
...
sun_pol_file = '_Ang_40_50_60.amu';
rec_pol_file = '_Ang_20_to_60_by_10.amu';
...
for zeta = 1:length(WS)
    ...
    [a_phy,a_CDOM,a_NAP,b_phy,bb_phy,b_NAP,bb_NAP,a_water,bb_water,b_water] = getAbsorptionBackscatteringVersion3(CHL,wav,ZETA);
    ...
    for x = 1:length(hydrosol_wave)
        if ~exist(strcat(RayXP_folder_path,filesep,'Scat_Mat_Normalized_',hydrosol_name,'_CHL_',num2str(CHL),'.',num2str(hydrosol_wave(x))),'file')
            ...
            [outFile] = Ray_XP_File_Converter_A1(scat_file,CHL,RayXP_file,RayXP_folder_path);
            ...
            scat_name{x} = strcat('Scat_Mat_Normalized_',hydrosol_name,'_CHL_',num2str(CHL),'.',num2str(hydrosol_wave(x)));
        end
    end
...
for zeta = 1:length(WS)
    % Creating model folders
    inp_fol_name = strcat('RayXP_WS_',num2str(WS(zeta)),'_V_',num2str(Version_num),'_FULL_ABS');
