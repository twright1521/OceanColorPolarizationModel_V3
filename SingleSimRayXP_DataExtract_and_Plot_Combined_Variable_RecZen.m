...
% this new case as well as the original
...
% Radiance, Q, U, V, Polarization, TauMol, TauSol,
% TauAbs, SSASol, A, B, and Bb

% This program specifically coded to plot changing Receiver Angles on a
% single graph.
% Does not account for other changing variables.
% Uses the first RayXP_CHL file in the sim folder.
...
sim_num = 602;        %Simulation Folder Number - can be a vector of simulation numbers

% Angles to Plot
% Should be found in the respective angle files used to create input files
Sun_Zen = 40;       % Sun polar angle, degrees                   
Rec_Zen = [20,30,40,50,60];       % Receiver polar angle, degrees (integer)
Rec_Azm = 90;       % Receiver azimuth relative to sun, degrees (integer) 

% Variables to Plot

plot_vars = {'Radiance','Polarization'};
...
for sim_idx = 1:length(sim_num)
    ...
    for rec_azm_idx = 1:length(Rec_Azm)
        for sun_zen_idx = 1:length(Sun_Zen)
            ...
            Sim_Data_Cell = cell(length(Rec_Zen),7);
...
%                     k = k + 1;
                    ...
                    for rec_zen_idx = 1:length(Rec_Zen)
                        ...
                        for y = 1:9
                            ...
                            if ( y == 3)
                                ...
                                if ( y == 9)
                                    Sim_Data_Cell{rec_zen_idx-k,cellnum} = str2double(test_str);
                                    cellnum = cellnum + 1;
                                else 
                        ...
                        Sim_Data_Cell{rec_zen_idx-k,3} = Sun_Zen(sun_zen_idx);
                        Sim_Data_Cell{rec_zen_idx-k,4} = Rec_Zen(rec_zen_idx);
                        Sim_Data_Cell{rec_zen_idx-k,5} = Rec_Azm(rec_azm_idx);
                        ...
                        for y = 1:N
                            ...
                            for z = 1:9
                                ...
                                if (z == 1)
                                    ...
                                    C{1,y} = str2double(test_str);
                                else

                                    D{z+4,y} = str2double(test_str);
                                    C{z+4,y} = str2double(test_str);
                                end
                            end
                        end
                        ...
                        Sim_Data_Cell{rec_zen_idx-k,7} = D;
                        Sim_Data_Cell{rec_zen_idx-k,8} = C;
            ...
            % Save Data File - Not Necessary
%             save(strcat(sim_fol_name,filesep,'Sim_Data_SZ_',...
%                         num2str(Sun_Zen(sun_zen_idx)),'_RZ_',...
%                         num2str(Rec_Zen(rec_zen_idx)),'_RA_',...
%                         num2str(Rec_Azm(rec_azm_idx)),'.mat'),'Sim_Data_Cell')
            ...
            var_names = {'CHL', 'Wind Speed', 'Sun Zenith', 'Receiver Zenith',...
                        'Receiver Azimuth','SSA Ocean Atm','Wavelength','Radiance','Q',...
            ...
            Fig_Fol = strcat(Fig_Fol,filesep,'Combined');
            if (exist(Fig_Fol, 'dir') == 0)
                   mkdir(Fig_Fol)
            end
            ...
            for w = 1:meg
                ...
                for x = 1:y-1
                    ...
                    if (mod(x,2) && x ~= 1)
                        legend_cell{2*w-1} = sprintf('%s\n%s',legend_cell{2*w-1},...
                            strcat(legend_name{x},':',32,num2str(legend_array(x,w))));
                        legend_cell{2*w} = '';
                    else
                        legend_cell{2*w-1} = strcat(legend_cell{2*w-1},32,legend_name{x},':',32,num2str(legend_array(x,w)));
                        legend_cell{2*w} = '';
                    end
                end
            end
            
            %Single case legend cell
            
            legend_cell_2 = cell(1,meg);
            ...
            for w = 1:meg
                ...
                for x = 1:y-1
                    ...
                    if (mod(x,2) && x ~= 1)
                        legend_cell_2{w} = sprintf('%s\n%s',legend_cell_2{w},...
                            strcat(legend_name{x},':',32,num2str(legend_array(x,w))));
                    else
                        legend_cell_2{w} = strcat(legend_cell_2{w},32,legend_name{x},':',32,num2str(legend_array(x,w)));
                    end
                end
            end
            ...
            % Plotting without fully absorbing
            for x = 1:n
                ...
                for w = 1:meg
                    ...
                    temp_array_2 = Sim_Data_Cell{w,8};

                    for y = 1:size(temp_array,2)
                        wav_array(y) = temp_array{1,y};
                        var_array(y) = temp_array{idx,y};

                        wav_array_2(y) = temp_array_2{1,y};
                        var_array_2(y) = temp_array_2{idx,y};
                    end

                    plot(wav_array,var_array,'LineWidth',0.8,'Color',cmap(w,:))

                    clearvars temp_array temp_array_2
                end

                xlabel('Wavelength - nm')

                if (contains('Radiance',plot_vars{x}))
                ...
                for zeta = 1:size(Sim_Data_Cell,1)
                    temp_array_2(zeta) = Sim_Data_Cell{zeta,leg_idx(1)};
                end
                ...
                figname = strcat(plot_vars{x},'_',title_name{1},'_',num2str(title_array(1)),...
                                ...
                                '_',num2str(min(temp_array_2)),'_to_',num2str(max(temp_array_2)));

                savefig(strcat(Fig_Fol,filesep,'Comb_',figname))

            end
            
            % Plotting with fully absorbing case |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
            ...
            for x = 1:n
                ...
                for w = 1:meg
                    ...
                    plot(wav_array,var_array,'LineWidth',0.8,'Color',cmap(w,:))
                    plot(wav_array_2,var_array_2,'LineWidth',0.8,'Color',cmap(w,:),'LineStyle','-.')
                ...
                figname = strcat(plot_vars{x},'_',title_name{1},'_',num2str(title_array(1)),...
                                '_SZ_',num2str(Sun_Zen(sun_zen_idx)),...
                                '_RA_',num2str(Rec_Azm(rec_azm_idx)),'_',legend_name{1},...
                                '_',num2str(min(temp_array_2)),'_to_',num2str(max(temp_array_2)),'_FULLABS');

                savefig(strcat(Fig_Fol,filesep,'Comb_',figname))

            end

            clearvars -except sim_num Sun_Zen Rec_Zen Rec_Azm plot_vars m n sim_fol_name filename sun_zen_idx rec_azm_idx fignum 
